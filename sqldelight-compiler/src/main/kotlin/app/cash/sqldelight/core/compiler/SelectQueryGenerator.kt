...
class SelectQueryGenerator(
  ...
  /**
   * The exposed query method which returns the default data class implementation with custom keys.
   *
   * `fun selectForId(id: Int, customKeys: List<String>): Query<Data>`
   */
  fun defaultResultTypeFunctionWithCustomKeys(): FunSpec {
    val argNameAllocator = NameAllocator()
    val parametersAndTypes = query.parameters.map { argNameAllocator.newName(it.name, it) to it.argumentType() }

    val function = defaultResultTypeFunctionInterfaceWithCustomKeys(parametersAndTypes)
    val params = parametersAndTypes.map { (name) -> CodeBlock.of(name) }
    ...
    return function
      .addStatement(
        ...
        CodeBlock
          ...
          .add(
            if (params.isEmpty()) {
              CodeBlock.of("${query.name}WithCustomKeys(customKeys)")
            } else {
              params.joinToCode(", ", "${query.name}WithCustomKeys(", ", customKeys)")
            },
          )
          .add(trailingLambda)
          .build(),
  ...
  /**
   * The exposed query method which returns a provided custom type with custom query keys.
   *
   * `fun <T> selectForId(id, mapper: (column1: String) -> T, customKeys: List<String>): Query<T>`
   */
  fun customResultTypeFunctionWithCustomKeys(): FunSpec {
    val function = customResultTypeFunctionInterfaceWithCustomKeys()
    val dialectCursorType = if (generateAsync) dialect.asyncRuntimeTypes.cursorType else dialect.runtimeTypes.cursorType
    ...
    // Assemble the actual mapper lambda with custom keys support:
    // { cursor ->
    //   check(cursor is SqlCursorSubclass)
    //   mapper(
    //       resultSet.getLong(0),
    ...
    if (!query.needsQuerySubType()) {
      // No need for a custom query type, return an instance of Query with custom keys:
      // return Query(statement, customKeys.toTypedArray()) { resultSet -> ... }
      function.addCode(
        "return %T(%L, customKeys.toTypedArray(), $DRIVER_NAME, %S, %S, %S)%L",
        QUERY_TYPE,
        query.id,
        query.statement.containingFile.name,
        query.name,
      ...
      // Custom type is needed to handle dirtying events with custom keys, return an instance of custom type:
      // return SelectForIdWithCustomKeys(id, customKeys) { resultSet -> ... }
      function.addCode(
        "return %N(%L, customKeys)%L",
        "${query.customQuerySubtype}WithCustomKeys",
        query.parameters.joinToString { it.name },
        mapperLambda.build(),
      )
    }
  ...
  private fun customResultTypeFunctionInterfaceWithCustomKeys(): FunSpec.Builder {
    val function = FunSpec.builder(query.name).also(::addJavadoc)
  ...
  private fun defaultResultTypeFunctionInterfaceWithCustomKeys(params: List<Pair<String, TypeName>>): FunSpec.Builder {
    val function = FunSpec.builder(query.name)
      .also(this::addJavadoc)
    params.forEach { (name, type) ->
      function.addParameter(name, type)
  ...
  /**
   * A custom query subtype that supports custom query keys.
   *
   * ```
   * private inner class SelectForIdWithCustomKeysQuery<out T>(
   *   private val _id: Int,
   *   private val customKeys: List<String>,
   *   mapper: (Cursor) -> T
   * ): Query<T>(database.helper, selectForId, mapper)
   * ```
   */
  fun querySubtypeWithCustomKeys(): TypeSpec {
    val queryType = TypeSpec.classBuilder("${query.customQuerySubtype}WithCustomKeys")
      .addModifiers(PRIVATE, INNER)
    val constructor = FunSpec.constructorBuilder()
    ...
    // Add the custom keys property
    queryType.addProperty(
      PropertySpec.builder("customKeys", ClassName("kotlin.collections", "List").parameterizedBy(ClassName("kotlin", "String")))
        .initializer("customKeys")
        .build(),
    )
    constructor.addParameter("customKeys", ClassName("kotlin.collections", "List").parameterizedBy(ClassName("kotlin", "String")))
    ...
    // Add listener methods that use the custom keys
    queryType
      .addFunction(
        FunSpec.builder("addListener")
          .addModifiers(OVERRIDE)
          .addParameter("listener", QUERY_LISTENER_TYPE)
          .addStatement("driver.addListener(*customKeys.toTypedArray(), listener = listener)")
          .build(),
      )
      .addFunction(
        FunSpec.builder("removeListener")
          .addModifiers(OVERRIDE)
          .addParameter("listener", QUERY_LISTENER_TYPE)
          .addStatement("driver.removeListener(*customKeys.toTypedArray(), listener = listener)")
          .build(),
      )
