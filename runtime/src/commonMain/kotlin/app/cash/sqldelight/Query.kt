...
/**
 * A listenable, typed query generated by SQLDelight with custom query keys.
 *
 * @param RowType the type that this query can map its result set to.
 */
@Suppress("FunctionName") // Emulating a constructor.
fun <RowType : Any> Query(
  ...
  query: String,
  mapper: (SqlCursor) -> RowType,
  customKeys: Array<out String>? = null,
): Query<RowType> {
  return Query(identifier, customKeys ?: queryKeys, driver, "unknown", "unknown", query, mapper)
}
...
fun <RowType : Any> Query(
  ...
  return SimpleQuery(identifier, customKeys ?: queryKeys, driver, fileName, label, query, mapper)
}
